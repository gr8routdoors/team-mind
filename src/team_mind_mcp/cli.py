...
from team_mind_mcp.server import MCPGateway, PluginRegistry
...
async def run_ingest(db_path: Path, args: argparse.Namespace) -> int:
    ...
    for t in args.targets:
        ...
        if target_path.is_dir():
            if args.recursive:
                ...
                print(f"Skipping directory {target_path} since --recursive is off", file=sys.stderr)
    ...
    if bundle:
        print(f"Successfully broadcasted {len(bundle.uris)} items to plugins.", file=sys.stderr)
        return 0
    return 1
...
def main() -> int:
    ...
    # Start Command
    start_parser = subparsers.add_parser("start", help="Start the MCP stdio server")
    
    # Ingest Command
    ingest_parser = subparsers.add_parser("ingest", help="Bulk ingest files or URIs into the database")
    ingest_parser.add_argument("targets", nargs="+", help="One or more file paths, directories, or URIs")
    ingest_parser.add_argument("--recursive", action="store_true", default=True, help="Recursively walk directories")
    ingest_parser.add_argument("--exclude", type=str, action="append", help="Glob patterns to exclude")
    ...
    db_path = Path(args.db_path).expanduser().resolve() if args.db_path else get_default_db_path()
