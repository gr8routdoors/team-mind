...
- **Information Decay:** Plugin-declared decay half-life on record types, computed at query time. *(SPEC-004 — COMPLETE)*
...
- **Semantic Type Routing (Phase A):** Three-type model (semantic type, media type, record type), registration-time routing, available vs enabled activation model, semantic_types on IngestionEvent/EventFilter. *(SPEC-008 — COMPLETE)*
- **Record Type Rename (Phase B):** Renamed `doctype` → `record_type` throughout codebase — column, indexes, APIs, tests, and docs. *(SPEC-009 — COMPLETE)*
