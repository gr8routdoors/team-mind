...
- **LLM Background Reaper:** Async post-ingestion process that scans for near-duplicate content across different URIs and cross-document contradictions. Covers both semantic deduplication (same fact, different URI) and conflict detection (contradicting facts). First concrete driver: reconciling dangling `provides`/`consumes` edges in Service Profile (Phase 4). *(Future — see ADR-006)*
