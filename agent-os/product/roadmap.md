...
- **Document Segments:** Explicit parent-child hierarchy via `parent_id`. Parents are metadata-only containers, segments are searchable atoms. `save_parent` API, cascade deletes, aggregate scoring from children. Segment ordering guaranteed by autoincrement IDs. *(SPEC-011 — COMPLETE)*
- **Search Query Enhancements:** Semantic type filtering on `retrieve_by_vector_similarity` and `retrieve_by_weight` — the `semantic_type` column and index exist but had no query-time filter. `DocumentRetrievalPlugin` converted to `TenantStorageManager` for cross-tenant scatter-gather queries. Documentation updates across the board. *(SPEC-012 — IN DESIGN)*
