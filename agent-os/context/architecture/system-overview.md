...
- **Future: Reliability Seeding + Background Conflict Detection:** The original inline Librarian concept has been retired (ADR-006). Reliability is instead addressed via three-layer score seeding at ingestion time (SPEC-007) and an asynchronous background reaper for cross-document contradiction detection.
