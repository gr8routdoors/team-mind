...
First, check if `agent-os/standards/tech-stack.md` exists.
...
[Summarize the key technologies from tech-stack.md]
...
If they choose option 2, proceed to ask them to specify.

**If no tech-stack standard exists**, use AskUserQuestion:
...
- The `/shape-spec` skill will read these files when planning features, so having them populated helps with context
