...
def test_indexes_created(tmp_path):
    ...
    assert "idx_documents_doctype" in index_names
    assert "idx_documents_plugin_doctype" in index_names
    adapter.close()
...
def test_saved_record_contains_plugin_and_doctype(tmp_path):
    ...
    with adapter._conn:
        cursor = adapter._conn.execute(
            "SELECT plugin, doctype FROM documents WHERE id = ?", (doc_id,)
        )
        row = cursor.fetchone()
