...
def test_sqlite_save_payload(tmp_path):
    ...
    # Verify the document
    with adapter._conn:
        cursor = adapter._conn.execute("SELECT uri, metadata FROM documents WHERE id = ?", (doc_id,))
        row = cursor.fetchone()
...
def test_sqlite_retrieve_by_vector_similarity(tmp_path):
    ...
    doc1 = adapter.save_payload("doc1", {"name": "doc1"}, v1)
    doc2 = adapter.save_payload("doc2", {"name": "doc2"}, v2)
    doc3 = adapter.save_payload("doc3", {"name": "doc3"}, v3)
