...
async def test_retrieval_fetches_local_db_markdown(tmp_path):
    ...
    response = await plugin.call_tool("get_full_document", {"uri": "virtual://in-db-only"})
...
async def test_retrieval_handles_unreachable_uri_pointers(tmp_path):
    ...
    with pytest.raises(ValueError, match="Document no longer available at file://.*deleted.md"):
